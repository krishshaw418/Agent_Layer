...
export async function preflightCommand(model: string, job: Job, opts: PreflightOptions): Promise<Bid | null>;
export async function preflightCommand(model: string, jobFile: string, opts: PreflightOptions): Promise<Bid | null>;
export async function preflightCommand(
  ...
  if (typeof jobOrFile === "string") {
    ...
    job = jobResult.job;
    spinner.succeed(good(`Job loaded → ${getJobId(job)}`) + dim(`  task: ${job.plan.task.type}`));
  } else {
    // Programmatic path: job object passed directly from worker
    // No spinner needed — caller already knows the job
    job = jobOrFile;
    console.log(good(`  Job → ${getJobId(job)}`) + dim(`  task: ${job.plan.task.type}`));
  }

  console.log(
    dim(`  input: ${job.plan.task.input.type}`) +
    dim(`  ·  data: ${job.plan.task.input.data.slice(0, 60)}${job.plan.task.input.data.length > 60 ? "…" : ""}`) +
    (job.plan.context.length > 0 ? dim(`  ·  context: ${job.plan.context.length} turns`) : "")
  );
  console.log();

  // Deadline check
  if (isJobExpired(job)) {
    console.log(bad(`  ✘  Job ${getJobId(job)} has already expired — no bid will be placed\n`));
    return null;
  }

  const remainingMs = jobTimeRemainingMs(job);
  console.log(dim(`  ⏱  ${Math.round(remainingMs / 1000)}s remaining until deadline`));
  console.log();
  ...
  const capCheck = checkCapabilities(job.plan.task.type, job.plan.task.input, capabilities);
  ...
  const estimatedOutput = Math.min(rawOutputEst, job.max_token_amount);
  ...
  if (probe.ok) {
    ...
    if (timeEst.estimatedMs != null && timeEst.estimatedMs > remainingMs) {
      ...
      console.log(`${dim("  Time remaining  ")}  ${chalk.white(formatDuration(remainingMs))}`);
  ...
  if (opts.yes) {
    await executeRequest(model, prompt, job.max_token_amount);
    return bid;
  }
  ...
  if (!opts.estimateOnly) {
    ...
    if (proceed) await executeRequest(model, prompt, job.max_token_amount);
...
async function executeRequest(model: string, prompt: string, maxTokens: number): Promise<void> {
  console.log();
